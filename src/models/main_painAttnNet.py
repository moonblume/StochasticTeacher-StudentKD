...
class PainAttnNet(nn.Module):
    ...
    def extract_features(self, x):
        ...
        se_feat = self.seresnet(mscn_feat)
        transformer_feat = self.encoderWrapper(se_feat)
        transformer_feat_flat = transformer_feat.contiguous().view(transformer_feat.shape[0], -1)
        final_output = self.fc(transformer_feat_flat)
        return transformer_feat, final_output
